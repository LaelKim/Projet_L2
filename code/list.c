...
/*
t_level_list *create_level_list(int n) {
    int num_cells = pow(2, n) - 1; // Calcul du nombre de cellules
    int levels[num_cells];         // Tableau pour stocker les niveaux de chaque cellule
    memset(levels, 0, num_cells * sizeof(int));  // Initialisation du tableau à 0

    // Calcul des niveaux pour chaque cellule
    for (int step = 2; step <= num_cells; step *= 2) {
        for (int i = step - 1; i < num_cells; i += step) {
            levels[i]++;
        }
    }

    // Création de la liste à niveaux
    t_level_list *list = create_level_list(n); // Assumant que cette fonction existe déjà

    // Ajout des cellules dans la liste
    for (int i = 0; i < num_cells; i++) {
        t_cell *cell = create_level_cell(i + 1, levels[i]); // Assumant que cette fonction existe déjà
        insert_sorted(list, cell); // Assumant que cette fonction existe déjà
    }
...
t_cell *search_all_levels(t_level_list *list, int value) {
    int level = list->max_levels - 1;
    t_cell *next = list->head;
    t_cell *lastInferior = next;
    ...
    for (; level >= 0; --level) {
        ...
        if (next != NULL && next->value == value) {
            return next;  // Value found
        }
        next = (level > 0) ? list->head : lastInferior->next[0];
    }
